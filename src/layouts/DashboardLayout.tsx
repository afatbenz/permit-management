...
// Sesuaikan path dengan routes.ts yang baru
const NAV: NavItem[] = [
  { label: 'Home', icon: <LayoutDashboard className="h-5 w-5" />, path: '/dashboard' },
  { label: 'Permit Management', icon: <FileText className="h-5 w-5" />, path: '/permit' },
];

export default function DashboardLayout({ children }: { children: ReactNode }) {
  const { theme, toggle } = useTheme();
  const navigate = useNavigate();
  const location = useLocation(); // Mendapatkan URL saat ini secara reaktif
  ...
  const SidebarContent = (
    <div className="flex h-full flex-col">
      ...
      <nav className="mt-2 flex-1 space-y-1 px-3">
        ...
        {NAV.map((item) => {
          // Logika rute aktif yang dinamis dan akurat
